...
function ParentLoginContent() {
  ...
  const targetUrl = rawRedirect && rawRedirect.startsWith('/') ? rawRedirect : '/parent/dashboard';
  const isReviewRedirect = targetUrl.includes('/tutor/review');
  ...
  // If already logged in, redirect to target URL
  useEffect(() => {
  ...
  }, [router, authStatus, authSession, targetUrl]);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.parent) {
        ...
        // If parent has no phone number and no specific redirect was requested, prompt phone step
        if (!data.parent.phone && !rawRedirect) {
          setStep('PHONE');
  ...
  return (
    <div className="parent-login-wrapper">
      ...
      <div className="parent-split-container">
        ...
        <div className="parent-brand-panel">
          ...
          <p style={{ fontSize: '0.95rem', color: '#94A3B8', lineHeight: 1.6, maxWidth: '520px', marginBottom: '2.5rem' }}>
            Access verified educator assignments, post feedback, track monthly class logs, view test reports, and experience seamless home tuition coordination.
          </p>
        ...
        <div className="parent-auth-panel">
          <div className="parent-auth-inner">
            {/* Header Badge */}
            <div style={{
              display: 'inline-flex',
              alignItems: 'center',
            ...
            <h2 className="parent-auth-title" style={{ fontSize: '1.85rem', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-0.02em', margin: 0 }}>
              {step === 'PHONE' ? 'Complete Profile' : 'Parent Sign In'}
            </h2>
            
            <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', marginTop: '0.35rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
              {isReviewRedirect
                ? 'Sign in to post your verified review for this tutor. You will be redirected right back.'
                : step === 'PHONE'
            ...
            {/* Context alert if coming from review */}
            {isReviewRedirect && (
              <div style={{
                ...
                <span>✍️</span>
                <span>Once signed in, you will return directly to complete your educator review.</span>
              </div>
            )}
            ...
            {step === 'INPUT' && (
              <>
                  ...
                  <div className="form-group">
                    ...
                    <div style={{ position: 'relative' }}>
                      <input
                        ...
                        style={{ paddingLeft: '2.5rem', height: '48px', width: '100%', boxSizing: 'border-box' }}
                      />
                ...
                {/* Divider */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', margin: '1.5rem 0 1.25rem 0' }}>
                ...
                {/* Google 1-Click Login at the Bottom */}
                <button
                  type="button"
